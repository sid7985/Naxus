...
import {
  ArrowLeft, Plug, Search, ExternalLink, CheckCircle2,
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-glass-border">
        <div className="flex items-center gap-3">
          ...
          <Plug className="w-5 h-5 text-agent-coder" />
          <h1 className="text-sm font-semibold">Integrations Hub</h1>
          <span className="text-xs text-text-muted font-mono ml-2">{stats.connected}/{stats.total} connected</span>
        </div>
      ...
      {/* Category tabs */}
      <div className="flex items-center gap-1 px-5 py-2 border-b border-glass-border overflow-x-auto">
        {(Object.entries(CATEGORY_LABELS) as [CategoryFilter, string][]).map(([key, label]) => (
          <button
            key={key}
            onClick={() => setFilter(key)}
            className={`px-3 py-1.5 rounded-lg text-xs whitespace-nowrap transition-colors ${
              filter === key ? 'bg-glass text-white' : 'text-text-muted hover:text-white'
            }`}
          >
            {label}
          </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <AnimatePresence>
              {filtered.map((connector) => {
                ...
                return (
                  <motion.div key={connector.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    <GlassPanel
                      hover
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <div
                          ...
                          style={{ background: `${connector.color}12`, border: `1px solid ${connector.color}25` }}
                        >
                          {connector.icon}
                        </div>
